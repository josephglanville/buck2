...
#[test]
fn import_store_output_binds_staged_store_artifact() -> buck2_error::Result<()> {
    let content = indoc!(
        r#"
         def test(c):
             store_path = c.actions.store_path(
             ...
             out = c.actions.import_store_output(
                 store_path = store_path,
                 dir = True,
                 missing_hint = "hydrate the package first",
                 manifest = c.actions.write("manifest.json", "{}"),
                 package_name = "bash",
                 version = "5.3",
                 output = "out",
                 ...
                 runtime_store_outputs = [
                     "/pkgs/store/0123456789abcdef0123456789abcdef-bash-5.3",
                 ],
                 canonical_tree_hash = "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
             )
             return (out.basename, out.short_path)
         "#
    );
